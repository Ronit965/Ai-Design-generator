...
const AuthModal = ({ isOpen, onClose, onAuth }) => {
  ...
  const handleGoogleQuickSignIn = () => {
    if (!googleName.trim()) return setError('Please enter your name');
    if (!googleEmail.trim()) return setError('Please enter your email');
    if (!validateEmail(googleEmail)) return setError('Please enter a valid email');

    setIsLoading(true);

    const users = JSON.parse(localStorage.getItem('aimodel_users') || '[]');
    let existingUser = users.find((u) => u.email === googleEmail.toLowerCase().trim());
    ...
    const session = { id: existingUser.id, name: existingUser.name, email: existingUser.email };
    localStorage.setItem('aimodel_session', JSON.stringify(session));
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {/* Google-style fallback when no Client ID is set */}
            {!GOOGLE_CLIENT_ID && (
              <div className="auth-divider-section">
                <div className="auth-divider">
                  <span>or</span>
                ...
                {!showGoogleForm ? (
                  <button
                    ...
                    onClick={() => { setShowGoogleForm(true); setError(''); }}
                  >
                  ...
                  >
                    ...
                    <div className="auth-field">
                      <div className="field-icon"><HiEnvelope /></div>
                      <input
                        type="email"
